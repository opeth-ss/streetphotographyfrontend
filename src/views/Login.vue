      ...
      <div>
        <form @submit.prevent="login">
          ...
          <div class="flex justify-center mt-4 mb-6">
            ...
            <a class="font-medium no-underline ml-2 text-blue-600 hover:text-blue-800" href="/register">Create today!</a>
          </div>
          <Button 
            type="submit" 
      ...
      >
        ...
        <template #footer>
          <div class="text-center w-full">
            <Button label="Close" @click="closePopup" class="bg-blue-600 text-white rounded-md hover:bg-blue-700" />
          </div>
        </template>
      </Dialog>
    </div>
...
const login = async () => {
  ...
  try {
    const response = await fetch('http://localhost:8080/streetphotography/api/auth/login', {
      method: 'POST',
      headers: {
